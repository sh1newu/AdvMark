...
def main():
    ...
    project_name = "FIN"  # xception/resnet18/resnet50
    ...
    optim = torch.optim.Adam(params_trainable, lr=1e-4, betas=(0.5, 0.999), eps=1e-6, weight_decay=1e-5)
    # loss function
    ...
    load(fed, '/home/likaide/sda4/wxs/pycharm/tmp/pycharm_project_wxs/Detecors/FIN/experiments/JPEG/fed.pt')
    ...
    val_dataloader = DataLoader(train_dataset, batch_size=16, shuffle=False, num_workers=0, pin_memory=True)
    ...
    for epoch in range(1, epoch_number + 1):
        ...
        for step, (image, label) in enumerate(train_dataloader, 1):
            ...
            noised_images = noise_layer(encoded_images.clone())
            ...
            g_loss_on_encoder = criterion_MSE(encoded_images, images)
            ...
            g_loss = g_loss_on_encoder + g_loss_on_decoder + 0.01 * g_loss_on_detector
            g_loss.backward()
        ...
        with torch.no_grad():
            ...
            for step, (image, label) in enumerate(val_dataloader, 1):
                ...
                encoded_images, left_noise = fed(input_data)
                noised_images = test_noise_layer(encoded_images.clone())
                ...
                g_loss = g_loss_on_encoder_LPIPS + g_loss_on_decoder + 0.01 * g_loss_on_detector
